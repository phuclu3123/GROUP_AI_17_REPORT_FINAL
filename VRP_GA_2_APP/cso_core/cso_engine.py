...
class CSOEngine(QObject):
    ...
    def __init__(
        ...
        self.time_window_penalty_weight = time_window_penalty_weight
        self.invalid_edge_penalty = invalid_edge_penalty
    ...
    def _eval_cat(self, cat: Cat):
        ...
        evaluate(
            ...
            self.time_windows, self.service_times, self.use_tw, self.use_capacity,
            self.capacity_penalty_weight,
            self.time_window_penalty_weight,
            self.invalid_edge_penalty,
        )
    ...
    def _seeking_mode(self, cat: Cat) -> Cat:
        ...
        # Theo tài liệu Mục 1: Chọn ngẫu nhiên một vị trí từ SMP để di chuyển
        selected_candidate = random.choice(pool)
    ...
    def run(self):
        ...
        global_best_cat = best_init.copy()
        self.generation_done.emit(
            0, global_best_cat.fitness, global_best_cat.total_cost, global_best_cat.conflicts,
            global_best_cat.get_routes(self.depots, self.cost_matrix, self.city_index)
        )
        ...
        for gen in range(1, self.generations + 1):
            ...
            for cat in swarm[self.elitism_count:]:
                ...
                if cat.seeking:
                    ...
                    new_cat = self._tracing_mode(cat, leader_cat)
            ...
            if best.fitness > global_best_cat.fitness:
                global_best_cat = best.copy()

            self.generation_done.emit(
                gen, global_best_cat.fitness, global_best_cat.total_cost, global_best_cat.conflicts,
                global_best_cat.get_routes(self.depots, self.cost_matrix, self.city_index)
            )

        # Trả về kết quả
        self.finished.emit(global_best_cat.get_routes(self.depots, self.cost_matrix, self.city_index))
